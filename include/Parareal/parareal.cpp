 ...
 * @return 0 if the algorithm completes successfully.
 */
__global__ inline int 
gpus_parareal(ode_system &sys, time_stepper coarse, time_stepper fine, 
             int para_its, Eigen::MatrixXd &y_sol)
{
  // Get the thread index and stride for the GPU
  ...
  { // Begin Parareal Steps
    ...
    /* In parallel compute the fine iterates on top of the serial steps */
    //#pragma omp parallel for
    for (int n = idx; n < csteps-1; n += stride)
    //for (int n = 0; n < csteps-1; n++)
    { 
      /* Construct fine ODE from t_init+  [n,n+1] *dt */
      ode_system para = sys;
